class PizzaDelivery:
    ...
    def make_order(self) -> str:
        self.ordered = True
        ingredients_joined = ", ".join(f"{ingredient}: {quantity}" for ingredient, quantity in self.ingredients.items())
        return (f"You've ordered pizza {self.name} prepared with "
                f"{ingredients_joined} and the price will be {self.price}lv.")
